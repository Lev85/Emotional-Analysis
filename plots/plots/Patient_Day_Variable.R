...
# Filtramos el día escogido
datos_paciente<-datos_paciente[datos_paciente$weekday==dia,]


# Quito los nulos de la variable de interes
#datos_paciente<-na.omit(datos_paciente)
#datos_paciente<-head(datos_paciente,35)
...
plt<-plt + ggtitle(paste0(pltvariable," for patient ",pltpaciente," on ",dia,"s")) 
plt<-plt + geom_point(aes_string(y=pltvariable),size=0.7)  
plt<-plt + geom_smooth(data = datos_paciente,aes_string(x=pltdate,y=pltvariable),span=0.23,se=F)
#plt<-plt + scale_x_date(breaks = break.vec,date_labels = "%A %d %b %y")
plt<-plt + scale_x_date(breaks = break.vec,labels=datos_paciente$nw)
plt<-plt + xlab("week")
