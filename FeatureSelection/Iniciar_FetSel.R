...
fun_corr<-function(dcorval,ind_var){
  ...
  tryCatch(
    { 
      ...
      dtx<-corval[  !is.na(corval$final_diagnosis_numeric_cor) &
                    ( corval$final_diagnosis_numeric_cor<=-0.7 | 
                        corval$final_diagnosis_numeric_cor>=0.7)
                  ,c("var.x","final_diagnosis_numeric_cor")]
      cat("  \n")
      cat("  \n##### Correlation (top 10)  \n")
      cat("  \n")
      ...
      features<-dtx$final_diagnosis_numeric_cor
...
fun_chi <-function(dcorval,ind_var){
  ...
  tryCatch(
    { 
      ...
      dtx<-corval[ !is.na(corval$final_diagnosis_numeric_chi) & corval$final_diagnosis_numeric_chi>0.05 
                  ,c("var.x","final_diagnosis_numeric_chi")]
      ...
      features<-dtx$final_diagnosis_numeric_chi
      names(features)<-dtx$var.x
...
fun_anova<-function(dcorval,ind_var){
  ...
  tryCatch(
    { 
      ...
      dtx<-corval[!is.na(corval$final_diagnosis_numeric_anova) & corval$final_diagnosis_numeric_anova<=0.05 
                  ,c("var.x","final_diagnosis_numeric_anova")]
      ...
      features<-dtx$final_diagnosis_numeric_anova
      names(features)<-dtx$var.x
...
corval<-read.csv(file="fetselv2.csv",header=TRUE,sep=";",stringsAsFactors = FALSE)


patients<-c("CASGA","DOSI","ECSA","GIUS","MAPI","SAPE")
patients<-c("CASGA","ECSA","GIUS","MAPI","SAPE")
...
#featselalg<-c("CORR","CHI","ANOVA","BORUTA","LASSO","STEPWISE","RELIMP")
featselalg<-c("BORUTA")
...
for (pt in patients){
  ...
  rmarkdown::render('FeatSelv2.Rmd',  # file 2
                    output_file =  paste("FeatSel_", pt, ".docx", sep=''))
